...
class Coapcon(object):
    def __init__(self, transport: asyncio.Transport, protocol: CoapProtocol, clean_session: bool, keepalive: int):
    ...
    async def create_connection(cls, host, port, ssl, clean_session, keepalive, loop=None):
        ...
        transport, protocol = await loop.create_connection(CoapProtocol, host, port, ssl=ssl)
        return Coapcon(transport, protocol, clean_session, keepalive)
