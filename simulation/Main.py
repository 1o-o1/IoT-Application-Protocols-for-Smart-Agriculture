...
from Coap.protocol import CoapProtocol
from Coap.connection import Coapcon

from storage import HeapPersistentStorage
from latency import lat as late
from client_num import clientlat as clate
from ThroughPut import Throughput
logger = logging.getLogger(__name__)


number_sub =  int(input("Enter number of node: "))
message_len = int(input("Enter Length of message: "))
number_msg = int(input("Enter Number of message: "))
#number_host= 30
#number_sub =40
#message_len = 20
#number_msg = 1000
msg = np.random.randint(message_len,size=(number_msg));
print(msg)

def MQTT():
    msx = Message('mqttsn',msg);
    latency_s =0;
    for i in range(number_sub):
        cl = Client('mqttsn');
        latency_s = cl.lat()
    latency_m = msx.lat();
    latency = latency_m*latency_s;
    out =SubscriptionsHandler();
    thro = out.calculate_throughput('mqttsn');
    return latency,thro
...
class Message:
    ...
    def lat(self):
        latency = 0;
       # if self.topic=='mqtt':
        #latency = (self.payload_size**1.5)/45698+23+self.payload_size/456;
        obj =late(self.topic,message_len*number_msg)
        latency= obj.latency();
        return latency
...
def dds():
    ...
    latency = latency_m*latency_s;
    out =SubscriptionsHandler();
    thro = out.calculate_throughput('dds');
    return latency,thro
...
class SubscriptionsHandler:
    ...
    def calculate_throughput (self,protocol):
        objt= Throughput(protocol,number_msg)
        message_found = int(objt.actual())
...
def coap():
    ...
    latency = latency_m*latency_s;
    out =SubscriptionsHandler();
    thro = out.calculate_throughput('coap');
    return latency,thro
def http():
    ms2 = Message('http',msg);
    latency_s =1;
    for i in range(number_sub):
        cl = Client('http');
        latency_s = cl.lat()
    ...
    latency = latency_m*latency_s;
    out =SubscriptionsHandler();
    thro = out.calculate_throughput('http');
    return latency,thro
...
class Client(MqttPackageHandler, SubscriptionsHandler):
    ...
    def sent(self):
        print("clint "+str(self.id)+" is sent");
    @property
    ...
    def lat(self):
        objc=clate(self._client_id, number_sub)
        latency = objc.latency()/100
        return latency
    @property
    def properties(self):
...
latency,thrput=MQTT()
print("\n\n........MQTT-SN.........")
print("Latency is: "+str(latency) +"Milisec")
print("Througput is: "+str(thrput) +" message")
